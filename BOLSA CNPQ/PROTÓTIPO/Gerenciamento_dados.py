...
class GerenciamentoDados():
    @staticmethod
    def filtrar_ods(cidades_rs, ods):
        municipio = 'Município'
        coluna_score = f'Goal {ods} Score'
        colunas_ods = cidades_rs.filter(like=f'SDG{ods}_').columns
    ...
    @staticmethod
    def Criar_Graficos(ods):
        df = pd.read_excel("Planilhas/Base_de_Dados_IDSC-BR_2025.xlsx", sheet_name='Todos os Dados')
        livro = pd.read_excel("Planilhas/Base_de_Dados_IDSC-BR_2025.xlsx", sheet_name='Livro de Códigos')
        ...
        cidades_rs = df[df['UF'] == 'RS'].reset_index(drop=True)
        media_rs = cidades_rs[f'Goal {ods} Score'].mean(numeric_only=True)
        cidades_rs = cidades_rs[cidades_rs['Município'].isin(regiao_vales)].reset_index(drop=True)
        ...
        cidades_rs['Município'] = cidades_rs['Município'].str.strip()

        ods_escolhida = GerenciamentoDados.filtrar_ods(cidades_rs, ods)

        ods_escolhida = ods_escolhida.sort_values(by=f'Goal {ods} Score', ascending=False).reset_index(drop=True)
            
        santa_cruz = ods_escolhida[ods_escolhida['Município'] == 'Santa Cruz do Sul']
        ...
        ods_grafico = ods_grafico.sort_values(by=f'Goal {ods} Score', ascending=False)

        ods_grafico.plot(x='Município', y=[f'Goal {ods} Score'], kind='bar', figsize=(10, 6), color=['#1f77b4', '#ff7f0e'])

        plt.title('Pontuação da ODS')
        plt.ylabel('Puntuação')
        plt.xlabel('Cidade')
        plt.axhline(y=media_rs, color="g", linestyle='--', label="Média do estado")
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        plt.legend(loc='upper right')
        plt.tight_layout()
        plt.show()

        municipios = ods_escolhida['Município']
        ...
        cidades_rs = GerenciamentoDados.limpeza_dados(ods_escolhida, coluna_normalizados, livro, ods)
        normalizados = GerenciamentoDados.limpeza_dados(normalizados, coluna_normalizados, livro, ods)
        ...
        while i < tamanho_normalizados:
            ...
            media_rs = cidades_rs[titulo].mean(numeric_only=True)
            ...
            santa_cruz_normalizados = normalizados[normalizados['Município'] == 'Santa Cruz do Sul']

            if 'Santa Cruz do Sul' not in normalizados_grafico['Município'].values:    
            ...
            normalizados_grafico.plot(x='Município', y=titulo, kind='bar', figsize=(10, 6), color=['#1f77b4', '#ff7f0e'])

            plt.title(f'Pontuação do índice em 2025')
            plt.ylabel('Puntuação')
            plt.xlabel('Cidade')
            plt.axhline(y=media_rs, color="g", linestyle='--', label="Média do estado")
            plt.grid(axis='y', linestyle='--', alpha=0.7)
            ...
            plt.show()
            i+=1
        livro = livro[livro['ODS'] == ods]
        GerenciamentoDados.Baixar_df(ods_escolhida, livro)

    @staticmethod
    def limpeza_dados(df, colunas, livro, ods):
        livro = livro[livro['ODS'] == ods]
        ...
        i=0
        while i < len(painel):
            df.rename(columns={painel[i]:f"Painel do Índice: {indices[i]}"}, inplace=True) 
    ...
    def Definir_painel(cor):
        ...
        arquivo = os.path.join(downloads_path, "DataFrame da ODS.xlsx")
        df = pd.read_excel(arquivo, sheet_name='ODS_IDSC-BR_2025.xlsx')
    ...
    def Grafico_painel(df, pontuacao, indice):
            if df.empty:
                ...
                print(f"Para o Índice: {indice}")
            else:
                df.plot(x='Município', y=f'Índice: {indice}', kind='bar', figsize=(10, 6), color=['#1f77b4', '#ff7f0e'])
    ...
    def Grafico_painel_ODS(df, pontuacao, ods):
            if df.empty:
                print(f"Nenhum dado encontrado para a pontuação: {pontuacao} para a ODS {ods}")
            else:
                df.plot(x='Município', y=f'Goal {ods} Score', kind='bar', figsize=(10, 6), color=['#1f77b4', '#ff7f0e'])
    ...
    def LevantamentoDados(dado):
        ...
        df_rs = pd.read_excel("Planilhas/Base_de_Dados_IDSC-BR_2025.xlsx", sheet_name='Todos os Dados')
        df_rs = df_rs[df_rs['UF'] == "RS"]
        ...
        if dado == "media":
            ...
            df_rs = df_rs[f'Goal {ods} Score'].mean(numeric_only=True)
            if media >= 60:
                pontuacao = "Alta"
            ...
            tk.Label(caixa, text=f"{media}\nA média da pontuação das cidades é {pontuacao}. \n E a média do RS é {df_rs}", bg="#f0f0f0", font=("Times new roman", 11, "bold")).pack(pady=10)
        elif dado == "min":
            min = df.tail(1)
            max = df.head(1)
            minmax = pd.concat([min, max], ignore_index=True)
            
            minmax.plot(x='Município', y=f'Goal {ods} Score', kind='bar', figsize=(10, 6), color=['#1f77b4', '#ff7f0e'])
    ...
    def historicoCidades(cidade):
        ...
        dfs = {}
        for i in range(2, 5):
            dfs[f"df202{i}"] = pd.read_excel(f"Planilhas/Base_de_Dados_IDSC-BR_202{i}.xlsx",sheet_name="Todos os Dados")

    @staticmethod
    def Baixar_df(df, livro):
        downloads_path = os.path.join(os.path.expanduser("~"), "Downloads")

        file_path = os.path.join(downloads_path, f"DataFrame da ODS.xlsx")

        with pd.ExcelWriter(file_path) as writer:
            df.to_excel(writer, sheet_name=f"ODS_IDSC-BR_2025.xlsx", index=False)
